...
import SFPLogger, { ConsoleLogger, COLOR_KEY_VALUE } from '@dxatscale/sfpowerscripts.core/lib/logger/SFPLogger';
...
export default class PushSourceToOrg {
    ...
    async exec() {
        try {
            ...
            await new SourcePush(this.devOrg, true).exec(true,0,true);
            cli.action.stop();
        } catch (error) {
