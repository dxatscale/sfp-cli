...
export default class Version extends SfpCommand {
    ...
    if (mode === "ALL") {
      ...
      for (const packageDescriptor of projectConfig.packageDirectories) {
        ...
        if (packageDescriptor.dependencies) {
          this.updateVersionOfDependencies(
            packageDescriptor.dependencies,
            updatedPackageDescriptors
          );
        }
      ...
      const selectedPackageDescriptor = await new SelectPackageWorkflow(
        projectConfig
      ).pickAnExistingPackage();

      const newPackageVersion = await this.getNewPackageVersion(
        selectedPackageDescriptor
      );
   ...
   */
  private updateVersionOfDependencies(
    dependencies: any[],
    packageDescriptors: any[]
  ) {
    for (const dependency of dependencies) {
      ...
      const descriptor = packageDescriptors.find(
        (descriptor) => descriptor.package === dependencyName
      );
      if (descriptor) {
      ...
      if (pkgDir.package !== sfdxPackage) {
        if (pkgDir.dependencies) {
          ...
          pkgDir.dependencies.forEach((dependency) => {
            if (
              dependency.package === sfdxPackage ||
              pattern.test(dependency.package)
            ) {
  ...
  private async getNewPackageVersion(packageDescriptor): Promise<string> {
    const currentVersionNumber = new PackageVersion(
      packageDescriptor.versionNumber
    ).getVersionNumber();
    const incrementedMajorVersion = new PackageVersion(
      packageDescriptor.versionNumber
    ).increment(Positional.MAJOR);
    const incrementedMinorVersion = new PackageVersion(
      packageDescriptor.versionNumber
    ).increment(Positional.MINOR);
    const incrementedPatchVersion = new PackageVersion(
      packageDescriptor.versionNumber
    ).increment(Positional.PATCH);
